...
module.exports = (function (_$, name, options) {
	...
	//! notify 를 발생시킨다.
	const my_notify = (type, data, trace_id, notifier) => {
	 ...
	 * @returns {*}
	 */
	const my_notify_subscribe_sync = ($id, $node) => {
		...
		if(!id.startsWith(CONF_NS_NAME+':')){
			...
			// return Promise.all(CONF_CHILDS.map(child => child.do_subscribe(that)))
			// 	.then(() => that);
			let actions = CONF_CHILDS.map(child => child.do_subscribe(that));
		...
		//! inline function.
		let handler = {handle:null, promise:null};
		if (typeof params === 'function'){
			// _log(NS, '>> params is function');
			//! it must be promised handler.
			handler.callee = params;
			handler.handle = function(that){
	...
	if (CONF_MASTER)
	{
		...
		CONF_MASTER.do_subscribe(':record:*', (that) => {
	 ...
	 */
	const prepare_chain = (eid, $node, mode) => {
